...
exports.DiagnosticsAccessory = void 0;
...
const FAULT_SENSORS = ['oilLevel', 'coolantLevel', 'brakeFluid', 'washerFluid',
    'tyreFrontLeft', 'tyreFrontRight', 'tyreRearLeft', 'tyreRearRight'];
class DiagnosticsAccessory {
    constructor(platform, accessory, opts) {
        ...
        (0, accessoryInfo_1.setAccessoryInfo)(platform, accessory, 'XC90 — Diagnostics');
        ...
        const legacyFilter = accessory.services.find(s => s.subtype === 'serviceWarning' && s.UUID === Service.FilterMaintenance.UUID);
        if (legacyFilter) {
    ...
    calcServiceLifeLevel(timeToService, distanceToService) {
        const percentages = [];
    ...
    async poll() {
        try {
            ...
            for (const s of SENSORS) {
                ...
                if (s.type === 'leak') {
                    svc.updateCharacteristic(Characteristic.LeakDetected, warn ? Characteristic.LeakDetected.LEAK_DETECTED : Characteristic.LeakDetected.LEAK_NOT_DETECTED);
                }
                else {
                    svc.updateCharacteristic(Characteristic.ContactSensorState, warn ? Characteristic.ContactSensorState.CONTACT_NOT_DETECTED : Characteristic.ContactSensorState.CONTACT_DETECTED);
                }
            }
            // --- Summary: faults only, NOT service due ---
            ...
            if (summary) {
                summary.updateCharacteristic(Characteristic.ContactSensorState, anyFault
                    ? Characteristic.ContactSensorState.CONTACT_NOT_DETECTED
                    : Characteristic.ContactSensorState.CONTACT_DETECTED);
            }
            ...
            if (serviceSvc) {
                serviceSvc.updateCharacteristic(Characteristic.ContactSensorState, this.serviceDueOpen
                    ? Characteristic.ContactSensorState.CONTACT_NOT_DETECTED
                    : Characteristic.ContactSensorState.CONTACT_DETECTED);
            }
            this.platform.dbg(`Service Due: ${serviceLifeLevel}% remaining` +
            ...
            this.platform.log.warn('Diagnostics poll failed:', err.message);
        }
    }
}
exports.DiagnosticsAccessory = DiagnosticsAccessory;
//# sourceMappingURL=diagnosticsAccessory.js.map