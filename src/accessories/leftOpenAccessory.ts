...
function buildLabel(
  doors: { frontLeft: boolean; frontRight: boolean; rearLeft: boolean; rearRight: boolean; hood: boolean; tailgate: boolean },
  windows: Record<string, string>,
): string {
  const parts: string[] = [];

  const openDoorCount = [doors.frontLeft, doors.frontRight, doors.rearLeft, doors.rearRight].filter(Boolean).length;
  if (openDoorCount === 1) parts.push('Door');
  if (openDoorCount > 1)  parts.push('Doors');
  if (doors.hood)         parts.push('Hood');
  if (doors.tailgate)     parts.push('Tailgate');

  const openWindowCount = ['frontLeft', 'frontRight', 'rearLeft', 'rearRight']
    .filter(k => windows[k] === 'OPEN').length;
  if (openWindowCount === 1) parts.push('Window');
  if (openWindowCount > 1)  parts.push('Windows');
  if (windows['sunroof'] === 'OPEN') parts.push('Sunroof');

  return parts.length > 0 ? `Volvo: ${parts.join(' · ')}` : BASE_NAME;
}

export class LeftOpenAccessory {
  ...
  constructor(
    ...
    setAccessoryInfo(platform, accessory, 'XC90 — Left Open');
  ...
  private async poll(): Promise<void> {
    ...
    try {
      ...
      const anyDoorOpen =
        status.doors.frontLeft || status.doors.frontRight ||
      ...
      this.platform.log.warn('Left Open poll failed:', (err as Error).message);
    }
  }
}
